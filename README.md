...
1.  Copy `.env.example` located at the root of the project, and replace your the `BASEURL` for you API url. Change `NODE_ENV` file according to your environment (development or production).
2.  Rename the file to `.env` if develop or `.env.production` if production.
...
- Added the project to netflify in case there is any problem with the set up. Link: https://karvi-page-test.netlify.app/
